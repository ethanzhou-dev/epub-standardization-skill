...
### 阶段 2：检索网络数据
1. **严格基于 ISBN 查询**：必须使用 `search_web` 工具，以阶段 1 提取的 ISBN 作为**唯一确定的搜索关键字**（例如：`search_web("<ISBN> 豆瓣读书")`）。绝不能仅凭“书名”进行泛搜索，以避免混淆不同译者、不同出版社的版本。
2. **提取对应版本的精准数据**：从对应 ISBN 的具体页面结果中提取以下字段：
   - **书名规范**：提取书名时，必须**彻底去除所有版次与冗余说明信息**（例如“（原书第2版）”、“(修订版)”、“【精装】”等）。对于带有副标题的书，保留“主标题：副标题”的格式（例如“关键对话：如何高效能沟通”）。重命名物理文件时，若遇到操作系统不支持冒号 `:` 的情况，请将冒号替换为全角冒号 `：`、破折号 `-` 或空格。
   - **简介 (`<dc:description>`)**：必须是该特定 ISBN 版本的官方内容简介（这很重要，因为不同译本可能有不同的译者序言或特定说明），去除所有推广链接。
   - **标签 (`<dc:subject>`)**：基于该具体版本提取 3-5 个标准的分类标签（如心理学、社会学）。
   - **评分 (`calibre:rating`)**：必须严格提取该特定 ISBN 版本的真实网络 10 分制评分，以反映该译本/版次的真实口碑。获取后直接写入该数字（**严禁除以2**！例如豆瓣特定版本为 6.0 就传 6.0，绝不能混用该书其它高分版本的评分）。
